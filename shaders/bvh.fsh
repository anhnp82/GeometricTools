...
//avoid branching, it can slow down by a factor of 2
...
vec3 computeNormal()
{
	...
	int idx = 0;
	while (idx < maxIndex)//maxIndex uniform => constant, but while loop is expensive
	{
		vec4 data1 = texelFetch(bvh_sampler, ivec2( mod(idx, bvh_tex_size), idx/bvh_tex_size ), 0);
		idx++;
		vec4 data2 = texelFetch(bvh_sampler, ivec2( mod(idx, bvh_tex_size), idx/bvh_tex_size ), 0);
		idx++;
		
		if (data1.w == 1) // current element is a bbox
		{
		...
		else
		{
			...
			vec3 moveVec = data2.rgb;
			float moveLength = data2.w;
...
vec3 reflectionMap()
{
	...
	envColor = mix(envColor, copper, MixRatio);
	return envColor;
}
*/
...
void main() {
	...
	//N = normalize(gNormal);
	//if (dot( N, (gl_NormalMatrix * orientation) ) > 0.00001) N = normalize(computeNormal());
	
	N = normalize(computeNormal());
