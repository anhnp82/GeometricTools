...
public:
    BufferObject()
    {
        ...
        if (openGLError() ) {
            std::cerr << "could not generate buffer!" << std::endl;
            mObjectName = 0;
            return;
        }
    }
...
public:
    ...
    inline BufferObject *getBufferObject () const { return mBuffer; }
    ...
    void setBufferObject( BufferObject *_pBuffer ) { mBuffer = _pBuffer; }
